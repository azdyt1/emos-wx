	...
	export default {
		...
		onShow: function() {
			...
			if (that.opt == 'insert') {
				...
				that.loadDeptData(that);
				that.loadRoleData(that);
				let data = {userId:that.userId};
				that.ajax(that.url.searchUserInfo, 'POST', data, function(resp){
					let result = resp.data.result;
					that.name = result.name;
					that.sexIndex = result.sex == '男'? 0:1;
					that.tel = result.tel;
					that.email = result.email;
					that.hiredate = result.hiredate;
					that.deptIndex = that.deptArray.indexOf(result.dept);
					that.statusIndex = result.status == '在职'?0:1;
